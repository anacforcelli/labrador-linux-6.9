...
static void __s700_usb3phy_param_setup(void __iomem *base, int is_device_mode)
{
	...
	set_reg32_val(base, (1<<7), (0<<7), USB3_PAGE1_REG0A);
	set_reg32_val(base, (1<<12), (1<<12), USB3_PAGE1_REG00);
	set_reg32_val(base, (0xF<<2), (0x0<<2), USB3_ANA0D);
	set_reg32_val(base, (1<<3), (1<<3), USB3_PAGE1_REG0B);
...
static int s700_usb3phy_param_setup(struct owl_usbphy *sphy, int is_device_mode)
{
	...
	if (is_device_mode) {
		dev_info(sphy->dev, "%s device mode\n", __func__);
	} else {
		dev_info(sphy->dev, "%s host mode\n", __func__);
...
static int s700_usb3phy_init(struct usb_phy *phy)
{
	...
	dev_info(phy->dev, "%s %d\n", __func__, __LINE__);
	/*owl_dwc3_usb3phy_param_setup(0);
	owl_dwc3_usb3phy_set_lpfs_det_mode(0);*/
