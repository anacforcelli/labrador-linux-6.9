...
static void
caninos_mmc_set_gpios(struct gpio_descs *gpios, bool val)
{
	int i;
	if (!IS_ERR_OR_NULL(gpios)){
		for (i = 0; i < gpios->ndescs ; i++) {
			gpiod_set_value_cansleep(gpios->desc[i], val); // this takes the logical value into account
		}
	}
}
...
static inline void caninos_mmc_power_off(struct mmc_host *host)
{
	...
	caninos_mmc_set_gpios(priv->en_gpios, false);
	caninos_mmc_set_gpios(priv->pwr_gpios, false);
	priv->enabled = false;
}
...
static inline void caninos_mmc_power_up(struct mmc_host *host)
{
	...
	if (!IS_ERR_OR_NULL(priv->en_gpios)) {
		mmc_delay(500);
	}
	
	caninos_mmc_set_gpios(priv->pwr_gpios, true);
	caninos_mmc_set_gpios(priv->en_gpios, true);
...
static int caninos_mmc_get_all_gpios(struct mmc_host *host)
{
	...
	priv->en_gpios = gpiod_get_array_optional(dev, "enable-gpios", 0);
	
	if (IS_ERR_OR_NULL(priv->en_gpios)) {

		if (IS_ERR(priv->en_gpios)) {
			dev_err(dev, "unable to get enable-gpios: error %ld\n", PTR_ERR(priv->en_gpios));
			return PTR_ERR(priv->en_gpios);
		} else
			dev_info(dev, "no enable-gpios");

	} else {
		for (i = 0; i < priv->en_gpios->ndescs; i++)
		{
			snprintf(buffer, sizeof(buffer), "sd%d-enable%d", priv->id, i);
			gpiod_direction_output(priv->en_gpios->desc[i], false);
		}
	}

	priv->pwr_gpios = gpiod_get_array_optional(dev, "power-gpios", 0);

	if (IS_ERR_OR_NULL(priv->pwr_gpios)) {

		if (IS_ERR(priv->pwr_gpios)) {
			dev_err(dev, "unable to get power-gpios: error %ld\n", PTR_ERR(priv->pwr_gpios));
			return PTR_ERR(priv->pwr_gpios);
		} else
			dev_info(dev, "no power-gpios");

	} else {
		for (i = 0; i < priv->pwr_gpios->ndescs; i++)
		{
			snprintf(buffer, sizeof(buffer), "sd%d-power%d", priv->id, i);
			gpiod_direction_output(priv->pwr_gpios->desc[i], false);
		}
	}
